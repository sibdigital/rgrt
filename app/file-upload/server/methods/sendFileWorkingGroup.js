...
Meteor.methods({
	async sendFileWorkingGroup(workingGroupId, store, file, msgData = {}) {
		if (!Meteor.userId()) {
			throw new Meteor.Error('error-invalid-user', 'Invalid user', { method: 'sendFileWorkingGroup' });
		}
		...
		Meteor.defer(() => callbacks.run('afterFileUpload', { user, workingGroupId, message: msg }));
