...
import { Agendas } from '../../../models/client';

export async function downloadAgenda({ _id, agendaData, dateString }) {
	if (!_id) {
	...
	doc.addSection({
		...
		children: sectionsRow,
	});

	const arr = agenda?.sections?.map((section) => new Paragraph({
		children: [
			new Paragraph({
				children: [new TextRun({
					// text: `${ section.item && section.item.trim() !== '' ? section.item :  'Пункт' }`,
